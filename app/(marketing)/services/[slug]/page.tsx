...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  const features = tServices.raw(`${slug}.features`) as string[];
  ...
  return (
    <article className="mx-auto max-w-[900px] px-4 py-12 sm:px-6 sm:py-16 lg:px-8 lg:py-24">
      ...
      <h1 className="font-[family-name:var(--font-display)] mt-6 text-3xl font-bold text-zinc-900 sm:mt-8 sm:text-4xl">
        {tServices(`${slug}.title`)}
      </h1>
      <p className="mt-6 text-lg leading-relaxed text-zinc-600">{tServices(`${slug}.description`)}</p>
      <h2 className="font-[family-name:var(--font-display)] mt-12 text-xl font-semibold text-zinc-900">
        {t("highlights")}
      </h2>
      ...
      <div className="mt-12 flex flex-wrap gap-4">
        ...
        >
          <Link href="/contact">{t("getConsult")}</Link>
        </Button>
